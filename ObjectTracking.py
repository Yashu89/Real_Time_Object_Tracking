    ...
    if contours:
        ...
        if cv.contourArea(c) > 500:
            ...
            if len(center_points) > 5:
                center_points.pop(0)
            smoothed_cx = int(np.mean([p[0] for p in center_points]))
            smoothed_cy = int(np.mean([p[1] for p in center_points]))

            cv.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv.circle(frame, (smoothed_cx, smoothed_cy), 5, (0, 0, 255), -1)

            if smoothed_cy > frame.shape[0]//2:
                cv.putText(frame, f"X:{smoothed_cx} Y:{smoothed_cy}", (x, y - 10),
                        cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            else:
                cv.putText(frame, f"X:{smoothed_cx} Y:{smoothed_cy}", (x, y + h + 20),
                        cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)    


            if smoothed_cx < frame.shape[1] // 3:
                command_h = "LEFT"
            elif smoothed_cx > 2 * frame.shape[1] // 3:
                command_h = "RIGHT"
            else:
                command_h = "FORWARD"

            if smoothed_cy < frame.shape[0] // 3:
                command_v = "UP"
            elif smoothed_cy > 2 * frame.shape[0] // 3:
                command_v = "DOWN"
            else:
                command_v = ""
